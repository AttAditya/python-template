# Python Repo Template

AttAditya's Python Repo Template is a boilerplate for Python projects, providing a structured and organized starting point for developers. It includes a predefined directory structure, essential files, and scripts to streamline the development process.

## Setup Checklist

- [ ] `config.sh`: set the variables about the development environment
- [ ] `pyproject.toml`: swap `<repo>` and `<description>` with the name and description of the repository
- [ ] `README.md`: swap `<repo>` with the name of the repository and update the description and other relevant sections
- [ ] Add secrets and variables in the GitHub Repository:
  - `secrets.AI_URL`
  - `secrets.AI_TOKEN`

## Installation

```sh
# Clone the repository
git clone https://github.com/AttAditya/<repo>.git
cd <repo>

# Source the development environment
source dev.sh

# Setup and run
dev setup
dev run
```

## About

This project is designed to facilitate the development of Python applications with a focus on modularity and scalability. It provides a clear separation of concerns, allowing developers to easily manage different components of their application.

## Project Structure

```tree
<repo>/
├── __init__.py
└── __main__.py
```

### Module Overview

> Unavailable

## Ecosystem Philosophy

> Unavailable

## Status

Experimental and under active development.  
APIs and structure may evolve as the project grows.

## Links

- [GitHub](https://github.com/attaditya/<repo>)
- [License](https://github.com/attaditya/<repo>/tree/main/LICENSE)

> _Made with <3 by [AttAditya](https://github.com/AttAditya)_
